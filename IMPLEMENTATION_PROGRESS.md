...
# 다음 작업: 파이썬 서버 도입 (계획 — 미착수, 2026-07-20 기록)

> MVP 이후 첫 확장. 목적: UE와 llama-server 사이(또는 옆)에 **파이썬 서버(FastAPI)**를 두어,
> 이후 **STT(음성→텍스트, 예: faster-whisper)** 등을 엔드포인트로 붙일 확장 지점을 만든다.
> 이 섹션만 읽고 다른 컴퓨터에서 이어서 작업할 수 있도록 결정사항/미결사항/실행 커맨드를 기록한다.

## 1. 아키텍처 후보 (미확정 — 착수 시 결정)
- **A. 프록시 방식 (권장)**: UE → 파이썬 서버(:8000) → llama-server(:8080).
  - UE는 Project Settings의 `ServerUrl`만 `http://127.0.0.1:8000/v1/chat/completions`으로 변경 (**UE 코드 변경 없음**).
  - 이후 `/stt`, `/tts`, 대화 요약 등을 같은 서버에 엔드포인트로 추가 — 확장 지점이 한 곳으로 모임.
- **B. 병렬 방식**: 채팅은 지금처럼 UE → llama-server(:8080) 직접 유지, 파이썬 서버는 STT 전용 별도 포트.
  - 기존 경로를 전혀 건드리지 않는 대신, UE가 서버 두 개를 알아야 함.

## 2. 1차 범위 (골격만 — STT 실연동은 그다음 단계)
- FastAPI 서버 골격 + `GET /health`.
- (프록시 채택 시) `POST /v1/chat/completions` — llama-server로 **그대로 중계**(passthrough).
- `POST /stt` 스텁 — 501 또는 `{"error": "미구현"}` 응답만 (엔드포인트 자리 확보).
- 실제 STT 모델(faster-whisper 등) 연동, 마이크 캡처(UE) 등은 이후 단계.

## 3. 예정 디렉터리 구조 (리포 루트)
```
python_server/
  main.py            # FastAPI 앱: /health, /v1/chat/completions 중계, /stt 스텁
...
```
- 사전 조건: Python 3.10+ 설치, llama-server 기동(§ 아래 5 참조).

## 5. 다른 컴퓨터에서 재개 시 필요한 것
- llama-server: `llama-b10038-bin-win-cuda-12.4-x64\llama-server.exe -m ..\model\gemma-4-E2B-it-Q8_0.gguf --host 127.0.0.1 --port 8080` (Phase 7 §6 참조).
- 모델 파일: `model/gemma-4-E2B-it-Q8_0.gguf` (용량 커서 git 미포함일 수 있음 — 별도 복사 필요).
- CUDA 12.4 빌드 폴더(`llama-b10038-bin-win-cuda-12.4-x64`) — GPU 없으면 CPU 빌드(`llama-b10034-bin-win-cpu-x64`)로도 동작(느림, 타임아웃 120s 내).
- UE 쪽 변경 지점: Project Settings > Game > Local LLM Settings의 **ServerUrl** (`Source/ChattingNPC/AIChat/LocalLLMSettings.h`의 `ULocalLLMSettings::ServerUrl`).

## 6. 주의사항 (이미 겪은 이슈 — 프록시 구현 시 반드시 반영)
- `gemma-4-E2B-it`는 **항상-생각(thinking) 모델**: max_tokens ≥ 512 필요 (UE가 보내는 값을 프록시가 **그대로 통과**시킬 것 — 줄이거나 기본값 덮어쓰기 금지).
- 응답에서 UE가 쓰는 것은 `choices[0].message.content` — 프록시가 응답 body를 변형하지 말 것.
- GPU 응답 ~9초, CPU ~36초 — 프록시 자체 타임아웃은 UE 타임아웃(120초)보다 길거나 없게.
- 한국어 요청/응답 — UTF-8 인코딩 유지.
