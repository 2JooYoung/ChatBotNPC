...
테스트할 때 서버를 둘 다 켜야 하는데(`llama-server` + `python 프록시`), 둘은 **하는 일도, 만든 주체도,
쓰는 기술도 다르다.**

| | **llama-server (:8080)** | **python 프록시 (:8000)** |
|---|---|---|
| 한 줄 역할 | NPC 대사를 **생성**하는 LLM 엔진 | 게임과 AI를 잇는 **중계·통합 허브** |
| 하는 일 | 언어모델(gemma-4-E2B-it) 실행 → 대사 텍스트 생성 | 채팅 중계 + 한국어 **음성 합성(TTS)** (+향후 STT) |
| 정체 | llama.cpp — **기성 C++ 바이너리**(수정 없이 실행) | **직접 만든 FastAPI 코드**(기능을 붙이는 곳) |
| 기술 | C++ / GPU 추론 | Python / MeloTTS 등 AI 생태계 |
| 게임이 아는가 | 몰라도 됨 | 게임은 **이 주소(:8000)만** 안다 |

**왜 하나로 안 합치나** — llama-server는 수정 불가한 기성 프로그램이라 거기에 TTS를 넣을 수 없다.
프록시는 내 코드라 TTS·STT·요약 같은 기능을 게임/LLM을 건드리지 않고 여기에만 추가한다.
그래서 게임(UE)은 주소 하나만 알면 되고, LLM이나 TTS 엔진을 **각각 독립적으로 교체**할 수 있다.

**비유** — llama-server = **주방**(요리=대사 생성), 프록시 = **홀 직원/프런트**(주문을 받아 주방에
전달 + 음성 포장 같은 부가 서비스), 게임 = **손님**(프런트하고만 대화한다).
